# Pitfall Practice: twelve standalone exercises

Twelve self-contained Python exercises that reinforce, through working code, the
themes of the whitepaper *Applied Generative AI: Twenty Pitfalls and Their
...
agents, MCP tool trust, end-to-end observability, chunking, and disk-based
vector search.

Every folder stands alone: its own `README.md`, `Dockerfile`, `setup.sh`,
`pyproject.toml`, `corpus/` documents, a commented CLI (`main.py`) and a pytest
suite that exercises the real application (no monkeypatching; tests that need
an API key skip cleanly when it is absent).
...
Folders `11` and `12` are the two the one-pager does not cover, parked at the
end so they sort last. `folder_numbering.txt` records the mapping, the
...
| 03_graphrag_fruit_graph | graphrag | Corpus-level questions via graph communities |
...
Exercises 11 and 12 are not on the ten-problem one-pager. Chunking was folded
...
Both exercises still stand on their own.
