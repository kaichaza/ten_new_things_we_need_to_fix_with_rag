...
uv sync
echo "NOTE: first run downloads the PromptInjection classifier (several hundred MB)."
echo "Run: uv run python main.py walk    Test: uv run pytest -q"
echo "Docker: docker build -t injection_defence . && docker run --rm --env-file .env injection_defence"
